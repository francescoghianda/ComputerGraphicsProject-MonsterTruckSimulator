...
struct Model {
	...
	void init(BaseProject* bp, std::vector<Vertex> vertices, std::vector<uint32_t> indices);
	void cleanup();
};
...
void Model::init(BaseProject* bp, std::vector<Vertex> vertices, std::vector<uint32_t> indices) {
	BP = bp;
	vertices = vertices;
	indices = indices;
	createVertexBuffer();
